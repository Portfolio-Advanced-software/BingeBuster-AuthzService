...
func DeleteAuthzByID(ctx context.Context, id string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to delete authz: %v", err)
	}

	// Check if the document exists
	if result.DeletedCount == 0 {
		return false, fmt.Errorf("authz with ID %s not found", id)
	}

	return true, nil
}

func DeleteAuthzByUserId(ctx context.Context, userId string) (bool, error) {
	...
	if result.DeletedCount == 0 {
		return false, fmt.Errorf("no authz records found for userID: %s", userId)
	}
